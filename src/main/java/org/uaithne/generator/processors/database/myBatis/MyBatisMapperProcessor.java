...
public class MyBatisMapperProcessor extends TemplateProcessor {
    ...
    public void processOperation(QueryGenerator sqlGenerator, OperationInfo operation, String namespace, Writer writer, boolean useAutoIncrementId, HashSet<EntityInfo> entitiesWithLastInsertedId) throws IOException {
        ...
        switch (operation.getOperationKind()) {
            ...
            case CUSTOM_INSERT_WITH_ID: {
                ...
                boolean useGeneratedKey = id.isIdentifierAutogenerated();
                if (useGeneratedKey) {
                    if (useAutoIncrementId && operation.hasReferenceToField(id)) {
                        operation.setReturnIdFromObjectWhenInsert(true);
                    } else {
                        String[] query = sqlGenerator.getEntityLastInsertedIdQuery(entity, operation);
                        if (query != null) {
